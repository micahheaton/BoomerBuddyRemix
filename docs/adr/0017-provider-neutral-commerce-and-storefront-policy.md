...
After the initial server-bound activation or trial establishes a service period, subscription-status events may restrict, shorten, or end access but may not extend paid-through access. Renewal or recovery requires an authenticated `invoice.paid` whose complete line collection contains exactly one non-prorated subscription-cycle line with the bound subscription, subscription-item lineage, configured price, and service period. That line period must exactly match the authenticated subscription snapshot before canonical contiguity checks apply. Invoice items, incomplete line collections, ambiguous or multiple subscription lines, unpaid invoices, and mismatched price, subscription, lineage, or period evidence fail closed.
...
Local tests cover raw-body signature pass/fail, timestamp/replay, wrong secret/environment, duplicate/out-of-order/versioned events, inbox atomicity, lifecycle normalization, overlaps, cancellation, grace, refund/dispute, reconciliation, allowance changes, and fail-closed access. Payment-proof regressions cover status-only non-extension, incomplete invoice lines, invoice-item rejection, exact subscription-item lineage, price and service-period matching, old invoices, and canonical period continuity. Mobile fixtures cover duplicate notifications, transaction identity, revocation, acknowledgement, and policy default-deny. An external evidence record must distinguish deterministic fixture proof from a provider-executed transaction.
