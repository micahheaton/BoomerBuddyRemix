...
Deployed release: `d0c22310de5ea0c4727035ca278f1a552c65eafb`
...
and worker health passed; the annual-first Family catalog, Learn, and public policy routes are live; and
...
Family monthly at USD 14.99 without a trial. Stripe initiation and purchasing remain disabled, so
neither visible offer can start Checkout, a trial, or a charge.
...
GitHub Actions run `33255158115` passed all five jobs after bounded browser reruns. It is pushed but
not merged, annotated-tagged, migrated, or deployed. Production remains on `d0c22310` and migration
0045. A later documentation and governance commit does not alter the `0059c4d` runtime identity or
expand its exact-SHA CI evidence.

Last historical candidate with complete local and GitHub CI evidence: `1fbd079de126aabccd788bfa3a854a77d1f6c1a3`

The integrated annual-trial, product-value, content, acquisition, and mobile candidate was committed
as `db2ff5efbc139623b72584f55e896213128c2552`, repaired as
...
`33176283345` passed all five jobs. Earlier commits `f4997946231b1548afc1a4b6d798ff05796a3c69`,
`11c0f70b2e293d71a09401b918b15a0a9ca2f711`, and
`08a285eaf39675b3daa1afef0568601525b8116f` remain immutable implementation baselines. This
versioned plan cannot bind its own final commit SHA because changing the plan changes the commit.
Release execution must derive the final exact SHA and annotated tag from Git after all changes are
committed, verify CI for that exact SHA, and record the result in an external release receipt.
...
deployment, testing, and controlled-launch work. No additional subjective founder phrase is an
active gate. Before any external or irreversible action, create a current scope receipt outside the
versioned candidate. The retired monthly-only noncharging template is historical format evidence and
must not govern the current catalog. The current receipt binds the final exact candidate SHA and
tree, green exact-SHA CI, annotated tag, ordered action manifest, exact target and
environment, safe account identifiers, cost cap if applicable, objective prerequisites, stop
conditions, and rollback. Advance `main` only if it preserves that exact candidate commit. A squash,
rebase, merge commit, changed tree, changed CI result, changed target, or changed scope requires a new
receipt and review.

Standing authorization is not evidence. It cannot replace direct customer consent, customer plan
choice or payment entry, a provider-required account-holder identity check or agreement, a qualified
legal or tax decision, credential custody, exact target proof, an exact-SHA deployment receipt, or a
tested rollback. Before each action, preserve secrets and customer PII, verify the target and
environment, capture evidence, and define containment. Agents may prepare and guide human-only steps
but may not impersonate another person, accept a provider agreement for the account holder, consent
for a customer, enter a customer's payment details, or fabricate evidence. Continue unrelated safe
lanes while one lane waits for an objective prerequisite.

Historical phrase-gate terminology is retained only so older evidence and regression checks remain
interpretable. That design used a planned annotated tag and said the first authorized action creates, pushes, and verifies the tag. Current execution preserves the exact-SHA and immutable-tag invariant,
but Section 1.1 does not require a new subjective phrase.
...
- The versioned production catalog controls the current Family annual and monthly catalog definitions. [OFFER-HYPOTHESIS-REGISTRY.md](./OFFER-HYPOTHESIS-REGISTRY.md) controls default-off Individual, group-rate, and referral hypotheses and preserves historical Family annual research rows. Individual offers remain default-off and referrals remain disabled. No repository entry alone authorizes production configuration, customer activation, live provider resources, or external action.
- Customer-facing web, HQ, mobile, email, store, and support copy may not contain U+2013 or U+2014.
- The current standing authorization in Section 1.1 supersedes broader historical phrase-gate wording. External actions still stop when their exact-candidate receipt, target proof, provider access, objective prerequisite, or rollback is absent.

## 3. Historical audit and current closure ledger

### 3.0 Controlling current ledger, 2026-08-29

Sections 3.1 through 3.10 preserve the `9b5d585` audit and later receipts. They are not a claim that any repository state is deployed or covered by provider, device, customer, or payment evidence. This ledger controls when historical wording differs from later evidence.

| Surface | Current evidence | Current disposition |
| --- | --- | --- |
| Historical immutable CI evidence | Candidate `1fbd079de126aabccd788bfa3a854a77d1f6c1a3` passed uninterrupted local verification, Playwright, and all four GitHub CI jobs. | Historical evidence only; it does not cover the later implementation baseline. |
| Deployed production baseline | Exact release `d0c22310de5ea0c4727035ca278f1a552c65eafb`, tree `b3f74c3ea858c0ce9b3d407a811327497af81248`, is under annotated tag `run3-1-replit-founding-household-d0c22310de5e`. Tag CI run `33238936758` passed all five jobs. All four BoomerBuddy 2.0 Replit services are deployed from that tag and production is at migration 0045. | This is the current noncharging runtime. It does not contain migration 0046 or the rejected-session recovery candidate and does not prove a real authenticated member, payment, signed device, or customer outcome. |
| Current runtime release candidate | Commit `0059c4dc07325fdcc7d36565480f1698d8f140de`, tree `bb5f73fa527dbfe9df71d72ddb0cda68b7f28ee8`, contains migration `0046_check_analysis_reuse.sql`, bounded Check reuse, and rejected-session recovery. Exact-SHA pull-request CI run `33255158115` passed all five jobs after bounded browser reruns. | Repository and CI evidence only. The candidate is pushed but is not merged, annotated-tagged, migrated, or deployed. It is not production or real-session proof. |
| Local validation receipt, 2026-08-26 | The large implementation tranche preceding `c39a98415320adb40737d1ea354674b2aa8c4194` passed: secrets across 799 text files; runtime dependency scope with 99 packages; all workspace and root typechecks; zero-warning lint and formatting; unit with 40 files and 433 tests; two bounded integration shards totaling 72 files and 472 tests; security with 54 files and 437 tests; the synthetic evaluation with 12 of 12 and zero forbidden-action violations; Playwright with 28 of 28; API, worker, web, HQ, and Expo web export builds; Expo Doctor with 21 of 21; deterministic mobile assets; provider-free distribution inputs; production auth-route resolution; production fail-closed UI; and `git diff --check`. The canonical-origin follow-up passed its focused 55 of 55 tests, and exact-SHA GitHub CI then passed all four jobs. The dependency audit reported 1 Low, 23 Moderate, and zero High or Critical findings. | This is local and exact-SHA CI evidence for the c39a984 implementation baseline, not managed PostgreSQL, restore, Stripe, Clerk, Replit deployment, signed native build, physical-device, store, customer, or payment evidence. This follow-up tranche needs its own applicable validation and CI. |
| Final release identity | The current runtime release candidate has an exact pushed SHA, tree, and green exact-SHA CI result, but no merge-preserving `main` identity or annotated release tag. | Preserve the exact candidate through merge, create and verify its annotated tag, and bind the release actions, targets, prerequisites, and rollback in an external receipt before migration or deployment. |
| Deployment topology | GitHub is source truth. The four BoomerBuddy 2.0 Replit consumers are web, API, worker, and HQ as mapped in Section 2; they pull exact commits and never push. All four currently run the exact `d0c22310` release. | Do not claim that a Replit service contains `0059c4d` until its pull-only deployment and service-specific receipt prove it. Keep the legacy project excluded. |
...
| Customer Clerk production instance | Root-domain Clerk infrastructure, including `accounts.boomerbuddy.net`, remains in place. A 2026-08-29 authenticated provider and service configuration session corrected Customer Home, unauthorized sign-in, sign-in, sign-up, sign-out, and Account Portal fallback routing; rotated and revoked the prior Google OAuth credential; aligned the web service's server and Customer production client Clerk keys; and successfully republished exact `d0c22310` web code. The sanitized external configuration receipt remains to be captured. A stale client session can still be rejected by server middleware and return to a hidden sign-in form; candidate `0059c4d` adds bounded rejected-session recovery. No real Google or email/password member journey is proved. | Preserve the corrected provider and service configuration. Merge, tag, and deploy the exact candidate only after its release receipt, then prove Google and email/password plus inbox or Device Trust handling, stable member reload, sign-out, re-entry, recovery, wrong-realm denial, and true billing re-verification without retaining PII. Allowed-subdomain restriction and genuine MFA remain separate open gates. |
...
| Production offer surface | Family annual at USD 149.90 after a seven-day trial is the deployed default catalog choice. Family monthly at USD 14.99 is the deployed no-trial alternative. Account creation alone does not start a trial or charge. Stripe initiation and purchasing remain disabled, so the visible offers are not purchasable. Payment remains web-first, with mobile P0 in parallel. | Individual monthly and annual remain implemented but default-off. Referrals remain disabled. Group-rate, coupon, credit, and native-purchase ideas remain unavailable until their separate promotion gates close. |
| Billing | Commit `f4997946231b1548afc1a4b6d798ff05796a3c69` completed the repository paid Family entitlement repair, and exact-SHA GitHub Actions run `33098426835` passed all five jobs. The retained 2026-08-24 live inventory found no BoomerBuddy 2.0 Products, Prices, Coupons, Promotion Codes, Subscriptions, webhooks, or Portal configurations; current live inventory is unknown until refreshed. Read-only sandbox access found zero 2.0 Products, Prices, Coupons, Promotion Codes, or Portal configurations plus exactly one enabled legacy webhook at `https://boomerbuddy.net/api/webhooks/stripe`. | Re-run the entitlement regressions on the final exact candidate and keep the legacy webhook untouched. Authentic isolated 2.0 sandbox resources and events, a separate endpoint under `api.boomerbuddy.net`, provider configuration, deployed preflight, Tax and receipt decisions, rollback, and first-payment evidence remain open. Do not let a repository pass substitute for those provider and live gates. |
| Support and feedback | Durable content-free support receipts, latest-state idempotent replay, tenant-scoped HQ handling, mobile retry classification, and production paid-entitlement feedback containment pass local automated tests in the c39a984 baseline. Public support, legal, accessibility, and account-deletion routes are deployed publicly in `d0c22310`. | Do not call Customer 1 ready until the content has professional approval, the support mailbox and provider URLs are operational, the exact candidate passes its release gates, and authenticated operational support plus genuine paid-access feedback are proved. Never store customer PII or free-text artifacts in general evidence. |
...
| External release setup | The user supplied standing authorization for in-scope launch work. The repository packet remains non-executable until its final exact SHA and tree, green CI, annotated tag, target accounts, ordered actions, objective prerequisites, stop conditions, and rollback are bound in an external receipt. | Continue all safe lanes. Stop an affected external action when its receipt, exact target, access, technical evidence, consent, qualified legal or tax decision, account-holder action, customer action, cost cap, or rollback is absent. Standing authorization does not convert an unknown into evidence. |
...
This table explains why the audited release was not launch-ready. Several repository blockers were closed in `1fbd079`; later closure work is represented by the clean pushed c39a984 implementation baseline and the controlling ledger in Section 3.0. The table is preserved for traceability and does not override the controlling ledger, current Git evidence, or the external release receipt.
...
That older evidence does not cover the later c39a984 auth-route recovery. The c39a984 baseline has local
route-resolution evidence, but neither candidate closes exact-SHA deployment, provider
home/fallback/path settings, allowed-subdomain proof, real Clerk post-auth routing, MFA enrollment,
billing reverification, native JWT, or physical-device gates.

### 3.6 Stripe account inventory

The original 2026-08-24 connection exposed the live account only. No sandbox or test-mode context
was available in that session, so it produced no authentic sandbox evidence. A later separate
read-only session inspected the provider context named `Boomer Buddy sandbox`; its facts are listed
separately below and do not retroactively change the original live-only receipt.
...
- In the later read-only session, the separate `Boomer Buddy sandbox` had zero Products, zero Prices, zero Coupons, zero Promotion Codes, zero Subscriptions, and zero Customer Portal configurations.
...
| Live Product and Prices do not exist | Billing owner | One Family Product, one USD 149.90 annual Price with a seven-day trial, and one USD 14.99 monthly Price without a trial after a verified write | Before live billing E2E and first payment | Stop if amount, currency, interval, trial behavior, quantity, or Product differs |
...
| Refund policy is not approved or aligned | Legal, billing, support, and product owners | One reviewed policy aligned across billing terms, pricing, Checkout disclosures, support runbooks, reconciliation, and refund authority | Before Checkout is exposed | Stop while the 30-day first-charge recommendation and the current generally-nonrefundable customer copy differ |
| Sandbox contains an enabled legacy-site webhook and no 2.0 commerce resources | Billing and API owners | In a separately identified BoomerBuddy 2.0 sandbox, create a separate webhook under `api.boomerbuddy.net` only after the exact-candidate, target, access, rollback, and coexistence gates pass; retain authentic Checkout, signed-webhook, lifecycle, refund, and reconciliation receipts with unambiguous endpoint, secret, event, and replay isolation | Before live configuration | Do not edit or delete the legacy webhook; fixtures, mocks, or events delivered to the legacy endpoint are insufficient |
| Current candidate is not merged, tagged, or externally release-bound | Launch integrator | Preserve candidate `0059c4dc07325fdcc7d36565480f1698d8f140de` and its green exact-SHA CI result through merge, record the exact `main` identity, create and verify an annotated tag, and bind ordered actions, exact targets, objective prerequisites, rollback, and digest in an external scope receipt | Before migration or deployment | Do not allow a squash, rebase, different merge result, unverified tag, changed tree, or historical receipt to substitute for the exact candidate |
| Customer Clerk provider routing is corrected but deployed end-to-end session proof is open | Identity, web, and platform owners | Preserve root-domain Clerk infrastructure and the corrected customer Home, sign-in, sign-up, sign-out, unauthorized, and Account Portal paths. Deploy the exact rejected-session recovery candidate, then prove Google plus email/password, inbox or Device Trust handling, stable member reload, sign-out, re-entry, recovery, and wrong-realm denial without PII. Enable allowed-subdomain restriction only after its separate before-and-after proof. | Before customer onboarding or billing | Stop on key drift, loop, hidden form, callback mismatch, 404, wrong `azp`, wrong realm, or unavailable rollback |
| Manually inserted production identity state is unverified | Identity, API, and data owners | Read-only, PII-free classification of any partial identity, issuer/subject conflict, or orphaned membership, followed by a separately reviewed remediation action if needed | Before retrying that identity or onboarding Customer 1 | Do not insert credentials or identity rows manually; stop on a partial record, issuer mismatch, ambiguous ownership, or unaudited database edit |
| Google OAuth credential rotation is complete but real Google session proof is open | Identity and security owners | Preserve the installed replacement and revoked prior credential evidence without recording either value. Prove Google sign-in, stable member reload, sign-out, re-entry, and rollback on the exact deployed candidate. | Before Customer 1 onboarding | Do not repeat, copy, commit, or reuse either credential; stop on OAuth, callback, realm, session, or rollback failure |
...
| Customer legal, support, accessibility, and deletion routes are deployed but not operationally or professionally approved | Product, ops, legal, and privacy owners | Professional content approval with versions and effective dates, operational support mailbox proof, provider URL configuration, exact-candidate E2E/accessibility evidence, authenticated workflow proof, and deployed route receipts | Before scheduling Customer 1 | Stop when approval, support operation, provider configuration, or any required route or workflow is unavailable |
...
| External release scope is not yet bound for the final exact SHA | Launch integrator | After paid-entitlement regression and green exact-SHA CI, complete the external `draft_scope` receipt with the exact target accounts, annotated tag, action manifest, scope digest, objective prerequisites, stop conditions, and rollback; verify the tag identity before the first provider write | Before merge/tag, any Replit pull/deploy, production migration, provider write, external message, or spend | A changed candidate, target, or scope requires a new receipt. Standing authorization does not replace customer consent, provider-required account-holder action, qualified legal or tax decisions, target access, or rollback proof |
...
### 3.8.1 Runtime candidate anchors for open external gates

These anchors are re-resolved against runtime release candidate
`0059c4dc07325fdcc7d36565480f1698d8f140de`. They prove repository behavior only; deployed and
provider facts remain separately classified.

| Open gate | Current exact anchors | What is proved and what remains open |
| --- | --- | --- |
| Real post-auth session and recovery proof | `apps/web/src/app/sign-in/client-trust/page.tsx:7-14`; `apps/web/src/app/sign-in/[[...sign-in]]/page.tsx:132-264`; `apps/web/src/lib/auth-recovery.ts:56-128`; `scripts/verify-next-resource-auth.mjs:25` | Deployed `d0c22310` resolves the Device Trust handoff route. Runtime candidate `0059c4d` adds bounded rejected-session recovery. Google, email/password, stable reload, sign-out, re-entry, and recovery remain unproved with a real provider session. |
| Family live initiation | `packages/config/src/index.ts:81-108`; `packages/config/src/index.ts:514-843`; `packages/persistence/migrations/0044_versioned_stripe_offer_catalog.sql:1-126` | Family annual at USD 149.90 after a seven-day trial and Family monthly at USD 14.99 without a trial are code-owned and visible. Stripe initiation remains disabled. Live Product, Price, Portal, webhook, account settings, first-payment, and rollback proof remain external. |
| Provider-backed paid entitlement | `packages/persistence/migrations/0035_run3_1_paid_family_catalog.sql:1-56`; `packages/persistence/migrations/0037_run3_1_paid_family_entitlement_repair.sql:63-333` | Production is migrated through 0045, including these Family catalog and exact provider-evidence migrations. Authentic provider records, lifecycle reconciliation, first payment, and timed application rollback remain unproved. |
| Lead receiver | `packages/config/src/index.ts:46-47`; `packages/config/src/index.ts:735-737`; `apps/web/src/app/pricing/page.tsx:10-17`; `apps/web/src/app/pricing/page.tsx:107-113` | Access-intent collection requires both default-off gates. A deployed receiver, operational owner, retention, response, and real lead evidence remain open. |
| Support intake | `packages/config/src/index.ts:48-50`; `packages/config/src/index.ts:628-630`; `packages/config/src/index.ts:740-742` | Customer access, HQ queue, and intake are separate default-off gates with dependency validation. Operational mailbox, staffing, deployed workflow, and alert evidence remain open. |
| Mobile identity and distribution | `apps/mobile/app.json:5`; `apps/mobile/app.json:12`; `apps/mobile/app.json:21`; `scripts/verify-mobile-distribution.mjs:12`; `scripts/verify-mobile-distribution.mjs:137-141` | Scheme and iOS/Android identifiers consistently use `net.boomerbuddy.app`. Provider collision checks, signing, native Clerk records, physical-device proof, and store receipts remain external. |
| Twilio containment | `packages/config/src/index.ts:97`; `packages/config/src/index.ts:364`; `packages/config/src/index.ts:614` | Runtime configuration accepts only disabled mode. No BoomerBuddy 2.0 messaging consent, compliance, provider, or delivery gate is closed. |
...
Use one Family plan with two clearly disclosed billing choices for the first paid cohort:

**Family annual: $149.90 after a seven-day trial, one household, guided setup included, cancel before the first charge.**

**Family monthly: $14.99 per month, one household, no trial, guided setup included, cancel any time.**

Account creation alone does not start a trial or charge. The customer must explicitly choose one
Family billing interval, consent to the disclosed amount and date, and provide a payment method in
Stripe Checkout. Do not introduce a coupon, referral credit, adaptive pricing, app-store purchase,
or additional paid tier for the first cohort. Individual offers remain default-off and referrals
remain disabled. Their research candidates stay governed by
[OFFER-HYPOTHESIS-REGISTRY.md](./OFFER-HYPOTHESIS-REGISTRY.md).
...
- Annual: cancel during the seven-day trial to avoid the first charge. Before activation, qualified review must settle renewal notice, cancellation timing, and any refund or prorating treatment beyond duplicate, unauthorized, erroneous, service-failure, and legally required refunds.
...
This recommendation is not approved customer policy. The current billing-terms source says monthly
charges are generally nonrefundable. Keep Checkout closed until qualified review selects one policy
and the billing terms, pricing, Checkout disclosures, support runbook, reconciliation, and refund
authority all match it.
...
The current public support source truthfully does not promise delivery, review, or a reply. Do not
replace that warning with the target above or call support operational until the mailbox, published
hours, accountable primary and backup, escalation path, and response drill are proved.
...
| Billing | Customer knowingly chooses Family annual at USD 149.90 after a seven-day trial or Family monthly at USD 14.99 without a trial; exact live Checkout completion and separate provider lifecycle evidence reconcile; canonical paid entitlement becomes active only from provider truth |
...
- **Account holder and launch operator:** provider-required identity, agreements, attestations, credential custody, support ownership, live-session observation, budget controls, and customer relationship actions.
...
Codex engineering may perform all standing-authorized in-scope work that does not require impersonating an account holder or customer. Account-holder references below identify an interactive provider-required step, legal or financial attestation, credential-custody action, or customer relationship action. They do not create a new subjective approval phrase.

### Workstream ownership and priority

Priority rule: P0 outranks launch-critical P1, which outranks other P1, then P2. Every owner works the highest unresolved gate in that order. Schedule pressure, sunk work, and provider timing never lower severity. Web-first payment is the revenue path. Mobile P0 work continues in parallel and is never silently deferred. Family annual at USD 149.90 after a seven-day trial is the deployed default catalog choice, and Family monthly at USD 14.99 remains available without a trial. Stripe initiation remains disabled, Individual offers remain default-off, referrals remain disabled, and Twilio remains disabled.
...
| --- | --- | --- | --- | --- |
| Day 0 | Freeze the Family annual-plus-monthly catalog and golden path. Fix lint. Create release branch. Read-only live Stripe inventory. Confirm no live Stripe catalog or enabled initiation. Start member, billing, mobile, operations, and copy tracks. Keep ad spend at $0. | Launch geography, support owner, refund recommendation, `net.boomerbuddy.app`, candidate availability | Exact backlog, owners, metrics, branch, test baseline, no untracked changes | Stop live path if offer, tax, or support cannot be stated truthfully |
...
| Day 4 | Prepare an exact candidate and a noncharging staging or initiation-disabled production-canary packet. When its exact-candidate receipt, target identity, access, migration, monitoring, and rollback gates pass, deploy it and prove real PostgreSQL concurrency, backup and isolated restore, worker heartbeat, raw-body webhook edge, alert receipt, support drill, incident stop, and rollback. Physical-device mobile test begins only after its provider gate. | Deployment target and immutable app identity are verified; provider settings remain noncharging; the scope receipt is complete before the first external write | Exact SHA has deployed receipts, restore proof, alert proof, browser proof, and mobile device evidence | Stop the affected lane on a missing receipt, target mismatch, auth, privacy, restore, accessibility, worker, or rollback failure |
| Day 5 | Run two clean synthetic first-customer rehearsals across separate customer and HQ sessions. Complete independent review. Freeze candidate. Prepare live product, price, portal, webhook, receipts, and statement descriptor plan but do not charge. | Qualified tax/accounting/legal disposition; support backup; recent operator MFA | Signed GO, NO-GO, or REMEDIATE dossier. GO names exact live writes and rollback | Any unresolved P0 or launch-critical P1 is NO-GO. Other P1 items require a named owner, dated closure plan, and bounded exposure. A code change invalidates affected rehearsal evidence |
| Day 6 | After the exact-SHA packet and all objective noncharging gates pass, perform only the reviewed setup: one Family Product with the exact annual and monthly Prices, trial settings, Portal, webhook, and deployment with Checkout initiation disabled. Verify zero-customer inventory. Opening a live initiation window and taking payment require separate first-charge evidence and direct customer action. | No account restriction; live secrets in approved custody; all objective closure gates passed; exact scope receipt recorded | Noncharging live readiness is green with no customer or money yet; kill switch and reconciliation remain active | Do not open Checkout from the noncharging setup. Disable new initiation first after any drift and keep webhook and worker running to reconcile an accepted action |
| Day 7 | Customer personally signs in, consents, completes setup and a useful Check, chooses a Family billing interval, enters payment in Stripe Checkout, receives provider confirmation, obtains canonical access according to provider truth, tests support, chooses feedback, and schedules follow-up. Observe the web path and offer signed mobile beta if ready. | Customer availability and direct consent; launch operator and backup present | Every row in Section 5 reconciles. A monthly customer has a paid invoice; an annual customer has a correctly scheduled seven-day trial and does not count as paid until `invoice.paid` after the trial. Day 1 and Day 7 follow-ups exist | Stop and refund or cancel per policy on wrong charge, auth mismatch, unsafe experience, support failure, or unresolved provider outcome |

### Honest seven-day risks

- The current repository sequence includes the paid Family entitlement repair and household safety beta, but it is not live-ready by code evidence alone. The later exact candidate still needs green CI, an annotated tag, an external scope receipt, authentic Stripe sandbox proof, live provider configuration, exact-SHA deployment, Clerk post-auth proof, operations and rollback evidence, signed-device evidence, and the controlled first-customer gate.
...
| Identity | Prove the rotated production Google OAuth configuration and real sign-in | Sanitized rotation and revocation receipt is captured, no secret value enters repository or evidence, and real Google sign-in plus rollback pass |
...
| Surface | Backlog item |
| --- | --- |
| Packaging | Keep Individual offers default-off and referrals disabled; continue Family annual and monthly provider-lifecycle proof before either live initiation path opens |
...
| Horizon | Target |
| --- | --- |
| Day 7 | One genuine household completes the chosen Family Checkout and value loop, with paid status only after `invoice.paid`; safe web fallback, signed internal mobile target, complete support, and reconciliation |
...
The user has supplied standing authorization for in-scope repository, provider-configuration,
deployment, testing, and controlled-launch work. No additional subjective founder phrase is required.
External execution remains bounded by an external scope receipt tied to the final exact SHA, tree,
green CI, annotated tag, exact target and environment, ordered action manifest, safe account
identifiers, cost cap if applicable, objective prerequisites, stop conditions, and rollback.
...
Agents may prepare, perform standing-authorized actions within provider access and policy, navigate
read-only surfaces, validate work, and document evidence, but may not impersonate the account holder,
customer, professional, or provider reviewer. If required participation, provider access, target
proof, objective prerequisite, rollback, or truthful evidence is unavailable, the affected external
lane stops while safe work continues.
