...
| Clean pushed implementation baseline | Candidate `9323bc5cdefb244a1f9be6f08e1f4007023ddc3c` contains the integrated canonical-origin validation, auth-route recovery, billing recovery, paid-feedback containment, support, mobile, copy, production-config hardening, test-runner hardening, and documentation closure work that preceded this correction. It was clean and pushed to `origin/codex/production-beta-launch`. | All four jobs in GitHub Actions run `32950774457` completed successfully for that exact SHA. It is not final, tagged, deployable, released, or inclusive of this documentation correction. |
| Local validation receipt, 2026-08-26 | Before `9323bc5cdefb244a1f9be6f08e1f4007023ddc3c` was pushed: secrets passed across 738 text files; runtime dependency scope passed with 99 packages; all workspace and root typechecks, zero-warning lint, and formatting passed; unit passed 36 files and 422 tests; the default bounded integration command passed two fresh-worker shards totaling 68 files and 456 tests; security passed 42 files and 316 tests; the synthetic evaluation passed 12 of 12 with zero forbidden-action violations; Playwright passed 26 of 26; API, worker, web, HQ, and Expo web export builds passed; Expo Doctor passed 21 of 21; deterministic mobile assets, provider-free distribution inputs, production auth-route resolution, production fail-closed UI, and `git diff --check` passed. The dependency audit reported 1 Low, 23 Moderate, and zero High or Critical findings. | This is local evidence for the 9323 implementation baseline, not a green CI, managed PostgreSQL, restore, Stripe, Clerk, Replit deployment, signed native build, physical-device, store, customer, or payment receipt. Any later documentation or code commit needs its own applicable validation and CI. |
...
This table explains why the audited release was not launch-ready. Several repository blockers were closed in `1fbd079`; later closure work is represented by the clean pushed 9323 implementation baseline and the controlling ledger in Section 3.0. The table is preserved for traceability and does not override the controlling ledger, current Git evidence, or the external release receipt.
...
That older evidence does not cover the later 9323 auth-route recovery. The 9323 baseline has local
