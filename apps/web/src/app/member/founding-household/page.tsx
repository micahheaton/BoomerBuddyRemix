...
function FoundingHousehold({
  ...
  async function withdraw() {
    ...
    try {
      ...
      const result = await apiRequest<OffboardFoundingHouseholdResponse>(
      ...
      setNotice(
        consentOnly
          ? 'Founding Household service consent was withdrawn. The already-ended sponsor chain was not changed again.'
          : `Founding Household service consent was withdrawn and only this sponsor grant was ${result.enrollment.state}.`,
      );
      await refreshPrincipal(selectedHouseholdId);
      await load();
    } catch (caught) {
    ...
    <main id="main-content" className="member-shell member-main">
      ...
      {!isAdministrator ? (
        <section className="card">
          ...
          <p>
            Protected-member, Trusted Circle, payer, and billing roles do not grant Founding
            Household acceptance authority.
          </p>
        </section>
      ) : status?.enrollment ? (
        <section className="card">
          <span className="dev-pill">{label(status.enrollment.state)}</span>
          <h2>{label(status.enrollment.benefitKey)}</h2>
          <p>
          ...
          {status.enrollment.accessAttentionCode ? (
            <p className="error" role="alert">
              Effective sponsored access is not active:{' '}
              {label(status.enrollment.accessAttentionCode)}. The ledger record is retained for
            ...
            <div className="form-stack">
              <label className="checkbox-row">
                ...
                {status.enrollment.ledgerState === 'active'
                  ? 'Withdraw Founding Household service consent and end only this sponsor grant.'
                  : 'Withdraw Founding Household service consent; founder offboarding already ended the sponsor grant.'}
              </label>
              <button
            ...
            <section className="card">
              ...
              <div className="form-stack">
                <label className="checkbox-row">
                  ...
                  and bounded operational-cohort telemetry consent only—not research, content reuse,
                  marketing, follow-up, referral, testimonial, or media consent.
                </label>
                <label className="checkbox-row">
                  <input
                    type="checkbox"
