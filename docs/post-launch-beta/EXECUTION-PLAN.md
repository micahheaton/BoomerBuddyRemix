...
| Final release identity is not externally bound | Launch integrator | Commit all approved changes, derive the exact SHA and annotated tag from Git, verify the applicable full local gate and green GitHub CI for that exact SHA, and record them in an external release receipt | Before merge, provider setup, or deployment | Do not self-bind a versioned file, reuse `1fbd079` or 9323 receipts for later changes, or claim run `32950774457` covers a later commit |
...
| Noncharging external release setup is not yet confirmed for the final exact SHA | Founder plus launch integrator | After paid-entitlement repair and green exact-SHA CI, derive the annotated tag and record the reviewed scope in an external release receipt; then the founder types `CONFIRM NONCHARGING RELEASE SETUP` in the active task | Before merge/tag, any Replit pull/deploy, production migration, provider write, external message, or spend | Continue safe local work only; the phrase does not authorize a charge, refund, customer consent, or an action outside the receipt |
