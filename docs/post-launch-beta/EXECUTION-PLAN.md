...
Clean pushed implementation and local-validation baseline before this follow-up tranche: `c39a98415320adb40737d1ea354674b2aa8c4194`. All four jobs in GitHub Actions run `33023999677` completed successfully for that exact SHA. This versioned plan cannot bind its own final commit SHA because changing the plan changes the commit. Release execution must derive the final exact SHA and annotated tag from Git after all approved changes are committed, verify CI for that exact SHA, and record the result in an external release receipt.
...
That phrase authorizes only the reviewed noncharging release-setup scope. Before requesting it, create
a `draft_pre_authorization` receipt outside the versioned candidate by using
[NONCHARGING-RELEASE-RECEIPT.md](./NONCHARGING-RELEASE-RECEIPT.md). The draft binds the final exact
candidate SHA and tree, green exact-SHA CI, planned annotated tag, ordered action manifest, and scope
digest while recording that the tag and every external effect are still absent. The founder cites the
receipt ID and digest with the phrase. The first authorized action creates, pushes, and verifies the
planned annotated tag on that same candidate, appends its tag object and peeled commit to the external
receipt, and only then permits the listed provider actions. Advance `main` only if it preserves that
exact candidate commit. A squash, rebase, merge commit, changed tree, changed CI result, or changed
scope invalidates the receipt and requires a new candidate and confirmation. This two-stage sequence
avoids asking a versioned packet or a not-yet-authorized tag to bind itself.

The phrase does not authorize a customer charge, refund, customer consent, customer plan choice,
customer communication, legal attestation, tax or bank attestation, or an action outside that receipt.
Before every external or irreversible action, the responsible agent must still verify the exact target
and environment, preserve secrets and customer PII, satisfy applicable legal and provider requirements,
capture evidence, define a bounded cap, and have a tested rollback or containment path. Agents may
prepare and guide human-only steps but may not impersonate another person or fabricate evidence.
...
| Clean pushed implementation baseline | Candidate `c39a98415320adb40737d1ea354674b2aa8c4194` contains the integrated canonical-origin validation, auth-route recovery, billing recovery, paid-feedback containment, support, mobile, copy, production-config hardening, test-runner hardening, and exact canonical GitHub checkout-origin acceptance that preceded this follow-up tranche. It was clean and pushed to `origin/codex/production-beta-launch`. | All four jobs in GitHub Actions run `33023999677` completed successfully for that exact SHA. It is not final, tagged, deployed, released, or inclusive of this follow-up tranche. |
...
| Billing | `c39a98415320adb40737d1ea354674b2aa8c4194` is not live-capable. The Family catalogue remains a `hypothesis`; Checkout can accept it, but provider-backed production entitlement verification cannot make it effective without a separately authorized repair. A real payment could reconcile without member access, so production payment remains NO-GO. Payment-action recovery, async Checkout, Portal/invoice guidance, and related local tests do not close that mismatch. Live Stripe remains empty. Read-only sandbox access works and confirms zero Products, Prices, Coupons, Promotion Codes, or Portal configurations plus exactly one enabled legacy webhook at `https://boomerbuddy.net/api/webhooks/stripe`. | Complete the separately authorized paid-entitlement repair, run its full local and CI gates on a later exact SHA, and keep the legacy webhook untouched. Only after all packet gates and authorization may a separate 2.0 endpoint under `api.boomerbuddy.net` be created; authentic sandbox resources/events, provider configuration, deployed preflight, Tax/receipt decisions, and first-payment evidence remain open. |
...
| External release setup | The confirmation phrase has not been recorded in the current evidence packet. The repository packet is a non-executable specification until paid-entitlement repair, exact final CI/tag evidence, and authorization are complete. | Continue safe local work. Stop all provider writes, merge/tag, Replit pull/deploy, production migration, external messages/spend, charges, and refunds until the exact phrase and an external release receipt bind the reviewed actions to the final exact SHA and annotated tag. |
...
| Final release identity is not externally bound | Launch integrator | Commit and push the approved candidate, record its exact SHA/tree, green exact-SHA CI, planned annotated tag, ordered action manifest, and digest in an external preauthorization receipt, obtain confirmation citing that receipt, then make tag creation/push/verification the first authorized action and append the tag object plus peeled commit before any provider write | Before merge/tag, provider setup, or deployment | Do not self-bind a versioned file, claim a planned tag already exists, allow a squash/rebase/different merge commit after binding, reuse `1fbd079` or c39a984 receipts for later changes, or claim run `33023999677` covers a later commit |
...
| Noncharging external release setup is not yet confirmed for the final exact SHA | Founder plus launch integrator | After paid-entitlement repair and green exact-SHA CI, complete the external `draft_pre_authorization` receipt with the planned tag and scope digest; the founder cites it and types `CONFIRM NONCHARGING RELEASE SETUP`; tag creation and verification are the first authorized action | Before merge/tag, any Replit pull/deploy, production migration, provider write, external message, or spend | Continue safe local work only; a changed candidate or scope requires a new receipt, and the phrase does not authorize a charge, refund, customer consent, or an action outside the receipt |
...
- The clean pushed c39a984 implementation baseline is not live-capable. It includes later billing recovery work, but the Family plan remains a hypothesis that provider-backed production entitlement verification cannot make effective. Paid-entitlement repair, a later exact-SHA local and CI gate, an annotated tag, and an external release receipt remain required.
