...
async function render(filename, input, size) {
  await sharp(input)
    .resize(size, size)
    .png({ compressionLevel: 9 })
    .toFile(`${assetDirectory}/${filename}`);
}
...
await Promise.all([
  ...
  render('adaptive-icon.png', svg(1024, shield('translate(282 255) scale(13.5)')), 1024),
  render('splash-icon.png', svg(1024, shield('translate(332 310) scale(10.6)')), 1024),
  render(
    'favicon.png',
    svg(
      256,
