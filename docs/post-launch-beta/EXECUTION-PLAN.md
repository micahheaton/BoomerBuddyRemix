...
### 3.0 Controlling current ledger, 2026-08-28
...
| Current pushed implementation sequence | Commits `db2ff5efbc139623b72584f55e896213128c2552` and `45714b713619e3d02785ca84669da6ec364d1b70` contain the integrated annual-trial, product-value, content, acquisition, mobile, and PostgreSQL verifier candidate. Pull-request GitHub Actions run `33176283345` passed all five jobs, and GitHub merged the sequence to `main` as `20fc6783046df682b92bb01495fabbff347d2727`. | Repository and CI evidence only. The merged candidate is not thereby tagged, deployed, provider-configured, signed for native distribution, customer-tested, or payment-proved. This documentation alignment creates a later exact candidate that needs its own applicable checks. |
...
| Final release identity | No versioned repository file can truthfully self-bind the final SHA that contains itself. | After all approved changes are committed, derive the exact SHA and annotated tag from Git, verify green CI for that SHA, and record them in an external release receipt before any release action. |
| Deployment topology | GitHub is source truth. The four BoomerBuddy 2.0 Replit consumers are web, API, worker, and HQ as mapped in Section 2; they pull exact commits and never push. | The user has standing authorization, but no Replit action proceeds without exact project identity, preserved local checkpoint evidence, a receipt-bound green candidate, migration and rollback readiness, and access to the correct four consumers. Do not assume any deployed surface contains a current candidate without an exact external deployment receipt. |
...
| Production offer candidate | Family annual at USD 149.90 after a seven-day trial is the intended default repository candidate. Family monthly at USD 14.99 remains available without a trial. Account creation alone does not start a trial or charge; the customer must explicitly choose Checkout, consent, and provide a payment method. Neither offer is live. Payment remains web-first, with mobile P0 in parallel. | Individual monthly and annual remain implemented but default-off. Referrals remain disabled. Group-rate, coupon, credit, and native-purchase ideas remain unavailable until their separate promotion gates close. |
| Billing | Commit `f4997946231b1548afc1a4b6d798ff05796a3c69` completed the repository paid Family entitlement repair, and exact-SHA GitHub Actions run `33098426835` passed all five jobs. Live Stripe remains empty. Read-only sandbox access confirms zero Products, Prices, Coupons, Promotion Codes, or Portal configurations plus exactly one enabled legacy webhook at `https://boomerbuddy.net/api/webhooks/stripe`. | Re-run the entitlement regressions on the final exact candidate and keep the legacy webhook untouched. Authentic isolated 2.0 sandbox resources and events, a separate endpoint under `api.boomerbuddy.net`, provider configuration, deployed preflight, Tax and receipt decisions, rollback, and first-payment evidence remain open. Do not let a repository pass substitute for those provider and live gates. |
| Support and feedback | Durable content-free support receipts, latest-state idempotent replay, tenant-scoped HQ handling, mobile retry classification, and production paid-entitlement feedback containment pass local automated tests in the c39a984 baseline. Public support, legal, accessibility, and account-deletion routes are implemented and linked locally. | Do not call Customer 1 ready until the content has professional approval, the support mailbox and provider URLs are operational, the final exact SHA passes CI, and deployed authenticated support plus genuine paid-access feedback are proved. Never store customer PII or free-text artifacts in general evidence. |
...
| Final release identity is not externally bound | Launch integrator | Commit and push the candidate, record its exact SHA and tree, green exact-SHA CI, annotated tag, ordered action manifest, exact targets, objective prerequisites, rollback, and digest in an external scope receipt; append the tag object plus peeled commit before any provider write | Before merge/tag, provider setup, or deployment | Do not self-bind a versioned file, claim a planned tag already exists, allow a squash/rebase/different merge commit after binding, reuse historical receipts for later changes, or claim an earlier CI run covers a later commit |
...
| Customer legal, support, accessibility, and deletion routes are implemented locally but not operationally approved or deployed | Product, ops, legal, and privacy owners | Professional content approval with versions and effective dates, operational support mailbox proof, provider URL configuration, exact-candidate E2E/accessibility evidence, and deployed route receipts | Before scheduling Customer 1 | Stop when approval, support operation, provider configuration, or any required deployed route is unavailable |
...
### 3.8.1 Current repository anchors for open external gates

These anchors describe the `08a285eaf39675b3daa1afef0568601525b8116f` implementation baseline.
They prove repository behavior only and must be rechecked after later edits.

| Open gate | Current exact anchors | What is proved and what remains open |
| --- | --- | --- |
| Live callback 404 | `apps/web/src/app/sign-in/client-trust/page.tsx:7-14`; `scripts/verify-next-resource-auth.mjs:25` | The repository contains an explicit dynamic Device Trust handoff route and compiled-route verification. The live 404 remains deployment and Clerk-routing evidence, not a missing-source claim. |
| Family live initiation | `packages/config/src/index.ts:88`; `packages/config/src/index.ts:520-556` | USD 14.99 monthly and a default-off live initiation control are code-owned. Live Product, Price, Portal, webhook, account settings, deployment, and first-payment proof remain external. |
| Provider-backed paid entitlement | `packages/persistence/migrations/0035_run3_1_paid_family_catalog.sql:1-56`; `packages/persistence/migrations/0037_run3_1_paid_family_entitlement_repair.sql:63-333` | The production Family catalog and exact provider-evidence checks exist in forward migrations. Managed production migration, provider records, reconciliation, and rollback remain unproved. |
...
| --- | --- | --- | --- | --- |
| Day 0 | Freeze the Family annual-plus-monthly catalog and golden path. Fix lint. Create release branch. Read-only live Stripe inventory. Confirm no live catalog. Start member, billing, mobile, operations, and copy tracks. Keep ad spend at $0. | Launch geography, support owner, refund recommendation, `net.boomerbuddy.app`, candidate availability | Exact backlog, owners, metrics, branch, test baseline, no untracked changes | Stop live path if offer, tax, or support cannot be stated truthfully |
